...
func (u UserRepositoryDb) GetUserWithToken(hash string) (*User, *errs.AppError) {
	...
	// mysql.Table("users").Select("users.name, emails.email").Joins("left join emails on emails.user_id = users.id").Scan(&results)
	//     if err := mysql.Table("ats").Joins(" INNER JOIN  read_states ON ats.id = read_states.target_id ").Where("AND read_states.status = 'unread'", tenantId, uid, common.EMPLOYEE)
	var user User
