...
func (u UserRepositoryDb) GetUserByToken(hash string) (*User, *errs.AppError) {
	...
	var user User
	result := u.client.Table("users").Select("users.id, users.first_name, users.last_name, users.email").
		Joins("inner join tokens on users.id = tokens.user_id").Where("tokens.token_hash = ? and t.expiry > ?", tokenHash[:], time.Now()).Scan(&user)
